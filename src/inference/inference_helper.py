...
DESCRIPTION - This script contains the helper functions used by the main inference script.
...
NODATAVALS = {'S2' : 0, 'CH': 255, 'ALOS': 1, 'LC': 255, 'DEM': -9999, 'LC': 255}
...
def load_train_config(ckpt_dir, arch, model_name, entity=None):
    """Return the training configuration for a model.

    Prefers the JSON sidecar saved next to the checkpoint at train time
    ({ckpt_dir}/{arch}/{model_name}_config.json); falls back to Weights & Biases for
    ...
    import wandb
    api = wandb.Api()
    wandb_name = get_mapping(api, arch)[model_name]
    return dict(api.run(f"{entity}/{arch}/{wandb_name}").config)
