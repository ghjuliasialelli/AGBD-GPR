...
This is a copy of kriging_gedi.py adapted to run on our own downsampled merged prediction (a single 2-band
GeoTIFF: band 1 = AGB, band 2 = STD), rather than on the CCI map. The input .tif and output file names are
driven by the --path_predictions and --model_name arguments passed from kriging.sh (downsampled=true, pred=ours).
...
def geographical_train_test_split(GEDI, model_name, order, width, height, val_size = 0.2, test_size = 0.3, stripe_size = 100, max_train_footprints = 25000, random_state = 42) :
    ...
    with open(f'splits/splits-{model_name}.pkl', 'wb') as f : pickle.dump(split_map, f)
...
def get_train_val_test_split(GEDI, model_name, pred_agb, pred_std, extra_ft, aux, norm_aux, norm_coords, path_dem, s2_tile, transform, upsampling_shape, width, height, run, test_holdout, val_holdout, max_train_footprints, max_split_diff, stripe_size, max_tries, test_indices = None, save_dir = './splits', SAVE = True) :
    
    # If it has already been computed, and subsampling or ood is not necessary, load them
    if SPLITS_EXIST :
...
if __name__ == '__main__':
    ...
    # Parse the arguments
    s2_tile, year, arch, ens_models, test_holdout, val_holdout, stripe_size, path_predictions, path_gedi, path_geometries, path_dem, path_kriging, aux, extra_features, norm_aux, norm_coords, norm_res, coords, pred_vals, matern_nu, num_iterations, pos_loss, lr, max_train_footprints, x_lengthscale, y_lengthscale, fix_x_y, z_aux_lengthscale, z_pred_lengthscale, eft_lengthscale, outputscale, gaussian_noise, learned_noise, model_name, patience, min_delta, COMPUTE_VAR, SAVE, SAVE_preds, max_split_diff, max_tries, _seed, composites, ood, agb = parser()
    ...
    fname = f"splits-{_id}_{test_holdout:.1f}_{val_holdout:.1f}_{max_train_footprints}_{max_split_diff:.1f}_{stripe_size}-{reference}.pkl"
    SPLITS_EXIST = isfile(join(save_dir, fname))
    subsample = subsample and (GEDI.shape[0] > 500)
    ...
    og_X_train, og_Y_train, og_X_val, og_Y_val = get_train_val_test_split(GEDI, model_name, pred_agb, pred_std, extra_ft, \
        ...
        val_holdout, max_train_footprints, max_split_diff, stripe_size, max_tries, test_indices = None, save_dir = save_dir, SAVE = SAVE)
    ...
    if SAVE :
        ...
        with open(join(tif_path, f"results-{model_name}-{reference}_{stripe_size}.pkl"), 'wb') as f:
    ...
    if SAVE_preds :
        ...
        with rs.open(join(tif_path, f"kriging-{model_name}-{reference}_{stripe_size}.tif"), 'w', **meta) as dst:
