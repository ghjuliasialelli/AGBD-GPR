...
This script computes the pre-kriging RMSE on all of the tiles, taking care of problematic footprints.

Run with the `krige` environment, on the cluster, with the following command:
    sbatch --wrap="python compute_pre_RMSE.py --model_name <model name> --force --test" --time=4:00:00 --mem-per-cpu=4G --cpus-per-task=1 --output=pre_RMSE.out --error=pre_RMSE.out
...
def _parser() :
    ...
    return args.model_name, args.force, args.subset, args.test
...
def compute_metrics(pre, ref) :
    """
    Computes the RMSE between the pre-kriging predictions and the reference values.

    Args:
    - pre (np.ndarray): The pre-kriging predictions.
    ...
    # Overall metrics
    rmse = np.sqrt(np.mean(np.power(pre - ref, 2)))
    me = np.mean(pre - ref)
    mae = np.mean(np.abs(pre - ref))
    num_footprints = len(pre)
    ...
    for lb, ub in zip(lbs, ubs) :
        ...
        if np.sum(bin_mask) > 0 :
            bin_rmse = np.sqrt(np.mean((pre[bin_mask] - ref[bin_mask]) ** 2))
...
def main() :
    ...
    model_name, force, subset, test_set = _parser()
    ...
    if test_set :
        import wandb; api = wandb.Api()  # W&B account required only for this lookup
        try: run = api.runs(f"{WANDB_ENTITY}/kriging", {"display_name": model_name + '-1'})[0]
        except: raise Exception(f"Could not find run with model name {model_name}-1 in the {WANDB_ENTITY}/kriging project.")
        config = run.config
        test_holdout = config['test_holdout']
        val_holdout = config['val_holdout']
        max_train_footprints = config['max_train_footprints']
        max_split_diff = float(config['max_split_diff'])
        stripe_size = config['stripe_size']
    
    save_path = join(path_kriging, 'predictions')
    file_name = f"pre{'_subset' if subset else ''}{'_test' + '-' + model_name if test_set else ''}.pkl"
            ...
            for tile in s2_tiles :
                try:
                    ...
                    if test_set :
                        ...
                        fname = f"splits-{tile_idx}_{test_holdout}_{val_holdout}_{max_train_footprints}_{max_split_diff}_{stripe_size}.pkl"
                        try:
                            with open(join(save_dir, fname), 'rb') as f: 
                                test_indices = pickle.load(f)['test']
                        except: # if couldn't load the file, get the config specifically for this run
                            run = api.runs(f"{WANDB_ENTITY}/kriging", {"display_name": model_name + f'-{tile_idx}'})[0]
                            config = run.config
                            test_holdout, val_holdout, max_train_footprints, max_split_diff, stripe_size = config['test_holdout'], config['val_holdout'], config['max_train_footprints'], float(config['max_split_diff']), config['stripe_size']
                            fname = f"splits-{tile_idx}_{test_holdout}_{val_holdout}_{max_train_footprints}_{max_split_diff}_{stripe_size}.pkl"
                            with open(join(save_dir, fname), 'rb') as f: 
                                test_indices = pickle.load(f)['test']
                    ...
                    res, binned_res = compute_metrics(pre[ok_mask], ref[ok_mask])
                    if res == {} : continue
                    ...
                    non_prob_pre, non_prob_ref = pre[ok_mask], ref[ok_mask]
                    non_prob_biomes = tile_biomes[ok_mask]
                    for biome in biomes :
                        biome_mask = (non_prob_biomes == biome)
                        if np.sum(biome_mask) > 0 :
                            biome_res, _ = compute_metrics(non_prob_pre[biome_mask], non_prob_ref[biome_mask])
